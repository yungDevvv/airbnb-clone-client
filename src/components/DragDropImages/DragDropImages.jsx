...
const DragDropImages = ({ setPreviewImages, validFiles }) => {
    ...
    useEffect(() => {
        if (files?.length) {
            const validFormats = validFiles.split(','); 
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                reader.onload = function () {
                    setPreviewImages(prev => [...prev, { id: uuid(), img: reader.result }]);
                };
    ...
    return (
        <div className="drag-and-drop" onDragLeave={handleOnDragLeave} onDragOver={handleDragOver} onDrop={handleDrop}>
            <div>
            ...
            <>
                <button type="button" onClick={() => inputRef.current.click()} className='button underline'>Upload from your device</button>
                <input type="file" accept={validFiles} multiple hidden ref={inputRef} onChange={(e) => setFiles(e.target.files)} />
            </>
        </div>
    )
}
